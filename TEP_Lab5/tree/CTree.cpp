...
CTree CTree::operator+(const CTree &other) const {
    ...
    if(!other.size) return copy;
    if(!copy.size) return other;

    CNode *firstLeaf = findFirstLeafToAdd(copy.root);

    *firstLeaf = *other.root->childrenTab[0];
...
CNode* CTree::findFirstLeafToAdd(CNode* parent){
    if (!parent->maxChildren || !parent->currentChildren)
        return parent;
